...
export function WorkspaceDashboard({ boards, allTasks, isLoading }: Props) {
  const stats = useMemo(() => {
    const total = allTasks.length;
    const completed = allTasks.filter((t) => t.progress === 100).length;
    const delayed = boards
      .flatMap((b) => b.columns.filter((col) => col.status === "delayed"))
    ...
    const statusData = Object.values(statusCounts).filter((d) => d.value > 0);

    // Priority distribution
    const priorityMap: Record<string, number> = { urgent: 0, high: 0, medium: 0, low: 0, none: 0 };
    allTasks.forEach((t) => { priorityMap[t.priority] = (priorityMap[t.priority] || 0) + 1; });
    const priorityData = Object.entries(priorityMap)
      .filter(([, v]) => v > 0)
      .map(([name, value]) => ({ name: name.toUpperCase(), value }));

    // Team workload across all boards
    const userMap: Record<string, UserWorkload & { progressSum: number }> = {};
    boards.forEach((b) => {
      b.columns.forEach((col) => {
        (col.tasks ?? []).forEach((task) => {
          const register = (id: string, name: string, email: string, color: string, progress: number) => {
            if (!userMap[id]) {
              userMap[id] = { id, name, email, color, total: 0, pending: 0, inProgress: 0, delayed: 0, completed: 0, avgProgress: 0, progressSum: 0 };
            }
            userMap[id].total += 1;
          ...
          if (task.assignments?.length) {
            task.assignments.forEach((a) => {
              const name = a.user.first_name ? `${a.user.first_name} ${a.user.last_name || ""}`.trim() : a.user.email;
    ...
    const teamWorkload: UserWorkload[] = Object.values(userMap)
      .map((u) => ({ ...u, avgProgress: u.total > 0 ? Math.round(u.progressSum / u.total) : 0 }))
      .sort((a, b) => b.total - a.total);

    // Per-board stats
    const boardStats = boards.map((b) => {
      const tasks = b.columns.flatMap((col) => col.tasks ?? []);
      const comp = tasks.filter((t) => t.progress === 100).length;
      const avg = tasks.length > 0
        ? Math.round(tasks.reduce((acc, t) => acc + (t.total_progress || t.progress || 0), 0) / tasks.length)
        : 0;
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center space-y-2">
          <LayoutDashboard className="h-10 w-10 text-muted-foreground mx-auto" />
    ...
    <div className="flex-1 overflow-auto bg-slate-50/50 p-6 space-y-6">
      ...
      <div className="flex items-center justify-end gap-2 no-print">
        <Button
          ...
          onClick={() => exportTasksCSV(allTasks, "workspace-tareas.csv")}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title="Total Tareas" value={stats.total}
          icon={<Clock className="h-5 w-5 text-blue-500" />}
          description={`En ${boards.length} tablero${boards.length !== 1 ? "s" : ""}`} />
        <KPICard title="Completadas" value={stats.completed}
          icon={<CheckCircle2 className="h-5 w-5 text-green-500" />}
          description={`${Math.round((stats.completed / (stats.total || 1)) * 100)}% del total`} />
        <KPICard title="Progreso Promedio" value={`${stats.avgProgress}%`}
          icon={<AlertCircle className="h-5 w-5 text-orange-500" />}
          progress={stats.avgProgress} />
        <KPICard title="Colaboradores" value={stats.teamWorkload.length}
          icon={<Users className="h-5 w-5 text-purple-500" />}
          description={stats.delayed > 0
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {stats.boardStats.map((b) => (
            <Link key={b.id} href={`/board/${b.id}`}>
              <Card className="shadow-none border border-border/50 hover:border-primary/40 hover:shadow-sm transition-all cursor-pointer">
                <CardHeader className="pb-2 pt-3 px-4">
                  <CardTitle className="text-sm font-semibold truncate">{b.name}</CardTitle>
                </CardHeader>
                <CardContent className="px-4 pb-3 space-y-2">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>{b.total} tarea{b.total !== 1 ? "s" : ""}</span>
                    <span className="font-bold text-foreground">{b.avgProgress}%</span>
                  </div>
                  <Progress value={b.avgProgress} className="h-1.5" />
                  <p className="text-[11px] text-muted-foreground">
                    {b.completed} completada{b.completed !== 1 ? "s" : ""}
                  </p>
                </CardContent>
              </Card>
            </Link>
...
function TeamWorkloadPanel({ data }: { data: UserWorkload[] }) {
  if (!data.length) {
    return <p className="text-center py-8 text-sm text-muted-foreground">No hay usuarios asignados en este workspace.</p>;
  }
  return (
    <div className="space-y-1">
      <div className="grid grid-cols-[minmax(140px,1fr)_48px_2fr_56px] gap-4 px-3 pb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground border-b border-border/50">
        <span>Usuario</span><span className="text-center">Tareas</span><span>Distribución</span><span className="text-right">Prog.</span>
      </div>
      {data.map((user) => (
        <div key={user.id} className="grid grid-cols-[minmax(140px,1fr)_48px_2fr_56px] gap-4 items-center px-3 py-2.5 rounded-lg hover:bg-muted/40 transition-colors">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="h-7 w-7 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ backgroundColor: user.color }}>
            ...
            {user.delayed > 0 && <AlertTriangle className="h-3.5 w-3.5 text-orange-500 flex-shrink-0" />}
          </div>
          <div className="text-center"><span className="text-sm font-bold tabular-nums">{user.total}</span></div>
          <div className="space-y-1">
            <div className="flex h-2.5 rounded-full overflow-hidden gap-px bg-muted/30">
              {user.pending > 0 && <div style={{ width: `${(user.pending / user.total) * 100}%`, backgroundColor: STATUS_BG.pending }} />}
              {user.inProgress > 0 && <div style={{ width: `${(user.inProgress / user.total) * 100}%`, backgroundColor: STATUS_BG.in_progress }} />}
              {user.delayed > 0 && <div style={{ width: `${(user.delayed / user.total) * 100}%`, backgroundColor: STATUS_BG.delayed }} />}
              {user.completed > 0 && <div style={{ width: `${(user.completed / user.total) * 100}%`, backgroundColor: STATUS_BG.completed }} />}
            </div>
            <div className="flex flex-wrap gap-x-3 text-[10px] text-muted-foreground">
              {user.pending > 0 && <span>{user.pending} pend.</span>}
              {user.inProgress > 0 && <span className="text-blue-600 dark:text-blue-400">{user.inProgress} en progr.</span>}
              {user.delayed > 0 && <span className="text-orange-600 dark:text-orange-400 font-semibold">{user.delayed} retr.</span>}
              {user.completed > 0 && <span className="text-green-600 dark:text-green-400">{user.completed} comp.</span>}
            </div>
          </div>
          <div className="text-right">
            <span className={cn("text-sm font-bold tabular-nums",
              user.avgProgress >= 75 ? "text-green-600 dark:text-green-400"
              : user.delayed > 0 ? "text-orange-600 dark:text-orange-400"
              : user.avgProgress >= 40 ? "text-blue-600 dark:text-blue-400"
              : "text-foreground")}>
              {user.avgProgress}%
            </span>
          </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 px-3 pt-3 border-t border-border/50 text-[11px] text-muted-foreground">
        {(["pending","in_progress","delayed","completed"] as const).map((s) => (
          <span key={s} className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full inline-block" style={{ backgroundColor: STATUS_BG[s] }} />
            {{ pending:"Pendiente", in_progress:"En Progreso", delayed:"Retrasado", completed:"Completado" }[s]}
          </span>
        ))}
      </div>
    </div>
...
interface KPICardProps {
  ...
  description?: string; progress?: number; alert?: boolean;
}
  ...
  return (
    <Card className={cn("shadow-none border transition-colors", alert ? "border-orange-300 dark:border-orange-800" : "border-border/50 hover:border-primary/30")}>
      ...
      <CardContent>
        ...
        {progress !== undefined && <div className="mt-3"><Progress value={progress} className="h-1.5" /></div>}
      </CardContent>
    </Card>
  );
}
